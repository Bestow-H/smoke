...
YOLOv11::YOLOv11(nvinfer1::ILogger& logger)
{
    std::string model_path;
    FileStorage fs("../src/yolo11/configureFiles.xml", FileStorage::READ);
    if (!fs.isOpened()) {
    throw std::runtime_error("无法打开配置文件");
    }
    fs["engineModel"] >> model_path;
    fs["confidenceThreshold"] >> conf_threshold;
...
void YOLOv11::postprocess(vector<Detection>& output)
{
    ...
    //存储所有吸烟的矩形
    vector<Rect> class1_boxes;
    //存储所有人的矩形
    vector<Rect> class0_boxes;
    ...
    for (int i = 0; i < nms_result.size(); i++)
    {
        ...
        results.conf = confidences[idx];
        results.bbox = boxes[idx];
        output.push_back(results);
        // 吸烟目标
        if(results.class_id == 1) {
            class1_boxes.push_back(results.bbox);
        }
        // 人脸目标
        if(results.class_id == 0) {
            // 保存坐标对
            class0_boxes.push_back(results.bbox);
        }
    }

    _result.clear();
    _result = vector<Point>(4, Point(0, 0));
    //find smoke
    if (!class1_boxes.empty()) 
    {
        Rect largestRect = findTheBest(class1_boxes);
        
        // 提取左上角和右下角坐标
        Point top_left(largestRect.x, largestRect.y);
        Point bottom_right(largestRect.x + largestRect.width, 
                        largestRect.y + largestRect.height);
        
        cout << "Largest Class 1 Box: "
            << "Top-Left: (" << top_left.x << ", " << top_left.y << "), "
            << "Bottom-Right: (" << bottom_right.x << ", " << bottom_right.y << ")" << endl;
        _result[2] = top_left;
        _result[3] = bottom_right;
    }
    //find face
    if (!class0_boxes.empty()) 
    {
        Rect largestRect = findTheBest(class0_boxes);
        ...
        cout << "Largest Class 0 Box: "
            << "Top-Left: (" << top_left.x << ", " << top_left.y << "), "
            << "Bottom-Right: (" << bottom_right.x << ", " << bottom_right.y << ")" << endl;
        _result[0] = top_left;
        _result[1] = bottom_right;
    }
}
...
// 输入图像
void YOLOv11::inference(cv::Mat& img)
{
...
Rect YOLOv11::findTheBest(const vector<Rect>& rects) {
    if (rects.empty()) {
        return Rect(); // 返回空矩形
    }
...
// 识别接口
std::vector<cv::Point> YOLOv11::getTarget()
{
    return _result;
}