...
int SerialTool::setSerialParam(int nSpeed,int nBits,char nEvent,int nStop,int fd){
...
int SerialTool::init(void*buffer,int size,const char*path,int nSpeed,int nBits,int nEvent,int nStop){
    ...
    // if(fd<0){
    if(openPort(path)<0){
	    printf("[SerialTool.cpp::init] 串口打开 Error");
        return 0;
    }
    //设置串口参数
    int i;
    if(0!=setSerialParam(this->nSpeed,this->nBits,this->nEvent,this->nStop,this->fd)){
        printf("[SerialTool.cpp::init] 初始化串口 Error");
        return 0;//设置串口参数失败
    }
    //开启串口线程
